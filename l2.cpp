...
constexpr int param_size = 100000 / 10;  // Number of model parameters 1199882 (MNIST)
...
// === GENERATE RANDOM MODELS AND ENCRYPT ===
    ...
    cout << "Time to encrypt: " << duration0.count() << " seconds.\n";
    ...
    for (int i = 0; i < num_clients; ++i)
    {
        vector<double> repeated_scalar(encoder.slot_count(), 3);
